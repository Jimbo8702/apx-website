...
- `src/lib/site-config.ts` — business facts: phone (551) 224-0345, info@apxcarsolutions.com, IG @apxcarnj, Dumont NJ, canonical URL (`https://www.apxcarsolutions.com`, env-overridable; www is canonical, apex 308s to www in Vercel), all 27 `serviceAreas`, `gtmId`. Read from here, don't hardcode.
...
src/
  app/
    ...
    about/              # founder story (owner Matt's copy); reuses ServiceHero + Quote;
                        #   _components/: founder-story, about-values
    services/
  ...
  components/
    ...
    sections/           # shared sections: quote (GoHighLevel form embed), legal-page, testimonials
...
- Page-specific components in the route's `_components/`; shared ones in `src/components/`. Server components by default; `"use client"` only where stateful (header, mailing-list). `quote.tsx` is now a server component (renders the GoHighLevel iframe embed).
...
- **Host canonicalization** (`next.config.ts`): both production Vercel aliases (`VERCEL_PROD_ALIASES` = `apx-website.vercel.app` + `apx-website-apx-mobile-detaling-s-projects.vercel.app`) 308 every path to `https://www.apxcarsolutions.com/:path*` so they stay out of the index and visitors land on the real domain. Exact host match, so preview/branch `*.vercel.app` deploys load normally — but a `headers()` rule stamps `X-Robots-Tag: noindex, nofollow` on **any** `*.vercel.app` host (`has` host regex `.*\.vercel\.app`), so previews and any future un-enumerated alias are never indexed while only the canonical www host is. **`www` is the canonical host** (the apex `apxcarsolutions.com` 308s to `www` in Vercel); `siteConfig.url` defaults to `https://www.apxcarsolutions.com` so all canonical tags / OG / sitemap / JSON-LD match the final non-redirecting URL. Production reads `NEXT_PUBLIC_SITE_URL` from the **Vercel dashboard** env (gitignored local `.env` does not deploy) — it must be `https://www.apxcarsolutions.com` or unset, never the non-www apex.
- **Third-party scripts**: LeadConnector (HighLevel) chat widget in `layout.tsx` (`<Script strategy="lazyOnload">`) and the Reputation Hub review widget on every page with a reviews section — the privacy policy doesn't mention LeadConnector yet, flag for the owner's next policy update. Google Tag Manager via `@next/third-parties` `<GoogleTagManager gtmId={siteConfig.gtmId}/>` (`GTM-NVWR3RF2`), placed high in the root layout with a hand-added `<noscript>` iframe fallback inside `<body>` (the component omits it). GA4 and the Meta Pixel are configured **inside the GTM container** (GTM dashboard), not in code — the policy's existing Google Analytics/GTM language covers GA4, but the Meta Pixel now firing through GTM is a new tracker to flag for the owner's next policy update. The policy also mentions a payment processor; none is wired — keep in sync if payments land. The privacy policy's contact block is verbatim doc text, not site-config.
