...
- **Host canonicalization** (`next.config.ts`, first redirect): the production Vercel alias `apx-website.vercel.app` 308s every path to `https://www.apxcarsolutions.com/:path*` so it stays out of the index and visitors land on the real domain. Exact host match, so preview/branch `*.vercel.app` deploys (Vercel noindexes those) load normally. **`www` is the canonical host** (the apex `apxcarsolutions.com` 308s to `www` in Vercel); `siteConfig.url` defaults to `https://www.apxcarsolutions.com` so all canonical tags / OG / sitemap / JSON-LD match the final non-redirecting URL. Production reads `NEXT_PUBLIC_SITE_URL` from the **Vercel dashboard** env (gitignored local `.env` does not deploy) — it must be `https://www.apxcarsolutions.com` or unset, never the non-www apex.
